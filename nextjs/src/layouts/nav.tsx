...
export default function Nav({
  ...
  const pagesCTAHTML =
    pages && pagesLocs && navPagesColor ? (
      <div className="items-center hidden space-x-4 font-medium lg:flex">
        {pages.map((page, i) => (
    ...
    pages && pagesLocs && navPagesColor ? (
      <nav
        ...
        >
      <ul className={"flex flex-col p-4 space-y-4 font-medium text-center " + navPagesColor}>
        {pages.map((page, i) => (
          <li key={i}>
        ...
        {cta && ctaLoc ? (
          <li>
            <Link key={ctaLoc} href={ctaLoc} className="btn btn-sm btn-primary">{cta}</Link>
          </li>
        ) : null}
      </ul>
      </nav>
  ...
  return (
    ...
    >
      <div className="items-center justify-between hidden w-1/2 h-full px-0 mx-auto text-sm lg:flex">
